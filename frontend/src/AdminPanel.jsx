...
const AdminPanel = () => {
  ...
  if (isLoading) return (
    <div className="flex flex-col items-center justify-center h-screen bg-notion-light dark:bg-notion-dark gap-4 transition-colors">
      ...
      <p className="text-sm font-medium text-notion-text-secondary animate-pulse uppercase tracking-widest">Cargando Usuarios</p>
    </div>
  );
        ...
        <div className="flex justify-between items-end mb-10">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-notion-text-secondary font-medium uppercase text-[10px] tracking-[0.3em]">Gestión de Accesos & Usuarios</p>
            </div>
          </div>
      ...
      <main className="max-w-6xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-700 delay-200">
         <div className="flex justify-between items-center mb-6 px-4">
            <div className="flex items-center gap-2 overflow-hidden">
               ...
               <span className="text-[10px] font-black text-notion-text-secondary uppercase tracking-[0.2em]">Listado de Acceso</span>
            </div>
            <div className="flex items-center gap-4">
              {isConfigured ? (
                <button 
                ...
                <div className="flex items-center gap-3 bg-amber-500/10 border border-amber-500/20 px-4 py-2.5 rounded-xl animate-pulse">
                  ...
                  <span className="text-[9px] font-black text-amber-500 uppercase tracking-widest">Configuración Requerida</span>
                </div>
              )}
              <button 
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-notion-border dark:border-white/5 bg-notion-bg-light dark:bg-white/[0.03]">
                ...
                <th className="py-5 px-4 text-[11px] font-black text-notion-text-secondary dark:text-white/40 uppercase tracking-widest text-center">Rol</th>
                <th className="py-5 px-4 text-[11px] font-black text-notion-text-secondary dark:text-white/40 uppercase tracking-widest text-center">Estado</th>
                <th className="py-5 px-4 text-[11px] font-black text-notion-text-secondary dark:text-white/40 uppercase tracking-widest text-center">Vinculación Notion</th>
                <th className="py-5 px-8 text-[11px] font-black text-notion-text-secondary dark:text-white/40 uppercase tracking-widest text-right">Acciones</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-notion-border dark:divide-white/5">
              {users.map((u) => (
                <tr key={u.id} className="hover:bg-black/[0.02] dark:hover:bg-white/[0.04] transition-all group">
                  <td className="py-6 px-8">
                    <span className="font-bold text-sm text-notion-text dark:text-white/90">{u.email}</span>
                  </td>
                  ...
                  <td className="py-6 px-4 text-center">
                    {u.role === ROLES.CLIENT ? (
                      <div className="flex flex-col items-center gap-1">
                        <code className="text-[10px] font-bold text-blue-500 bg-blue-500/5 px-3 py-1 rounded-lg border border-blue-500/10 uppercase tracking-wider">{u.external_client_id || '-'}</code>
                      </div>
                    ) : (
                      <span className="text-notion-text-secondary/20 dark:text-white/10 text-xs">-</span>
                    )}
                  </td>
                        ...
                        {u.role === ROLES.CLIENT && u.external_client_id && (
                          <button 
                            onClick={() => {
                              ...
                              const path = isProd ? `/test/frontend/view-as/${u.external_client_id}` : `/view-as/${u.external_client_id}`;
