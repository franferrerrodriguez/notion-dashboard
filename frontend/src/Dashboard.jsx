...
const Dashboard = () => {
  ...
  if (isLoading) return (
    <div className="flex flex-col items-center justify-center h-screen bg-notion-light dark:bg-notion-dark gap-4 transition-colors">
      ...
      <p className="text-sm font-medium text-notion-text-secondary animate-pulse uppercase tracking-widest">{t('notion_query')}</p>
    </div>
  );
          ...
          <div className="bg-white dark:bg-linear-to-br dark:from-[#202020] dark:to-[#1a1a1a] rounded-3xl p-10 border border-notion-border dark:border-white/5 shadow-2xl flex items-center justify-around relative overflow-hidden group transition-colors">
            ...
            <div className="relative w-40 h-40">
               ...
               <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                 <span className="text-4xl font-black tracking-tighter text-notion-text dark:text-white/90">{projects.length}</span>
            ...
            <div className="space-y-5">
              <LegendItem icon={<PieChart className="w-3 h-3" />} color={CHART_COLORS.NOT_STARTED} label={t('not_started')} count={stats.notStarted} t={t} />
              <LegendItem icon={<TrendingUp className="w-3 h-3" />} color={CHART_COLORS.IN_PROGRESS} label={t('in_progress')} count={stats.inProgress} t={t} />
              <LegendItem icon={<BarChart3 className="w-3 h-3" />} color={CHART_COLORS.COMPLETED} label={t('completed')} count={stats.completed} t={t} />
            </div>
          </div>
        </div>
      </header>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-notion-border dark:border-white/5 bg-notion-bg-light dark:bg-white/[0.03]">
                    ...
                    <th className="py-5 px-4 text-[11px] font-black text-notion-text-secondary dark:text-white/40 uppercase tracking-widest text-center">{t('col_phase')}</th>
                    <th className="py-5 px-4 text-[11px] font-black text-notion-text-secondary dark:text-white/40 uppercase tracking-widest text-center">{t('col_status')}</th>
                    <th className="py-5 px-8 text-[11px] font-black text-notion-text-secondary dark:text-white/40 uppercase tracking-widest text-center">{t('col_billing')}</th>
                  </tr>
                </thead>
                    ...
                    >
                      <td className="py-6 px-8">
                        <div className="flex items-center gap-4">
                          ...
                          <span className="font-bold text-sm text-notion-text dark:text-white/90 group-hover:text-blue-500 transition-colors">{p.name}</span>
                        </div>
                      </td>
...
const LegendItem = ({ color, label, count, icon, t }) => (
  <div className="flex items-center gap-4 group cursor-default">
    <div className="w-8 h-8 rounded-xl flex items-center justify-center transition-all bg-black/[0.02] dark:bg-white/[0.03] border border-notion-border dark:border-white/5 group-hover:border-notion-text-secondary/30 dark:group-hover:border-white/20 group-hover:bg-black/5 dark:group-hover:bg-white/5" style={{ color }}>
      {icon}
    </div>
    <div className="flex flex-col">
      <span className="text-[10px] font-black text-notion-text-secondary uppercase tracking-widest leading-none mb-1 group-hover:text-notion-text dark:group-hover:text-white/70 transition-colors">{label}</span>
      <span className="text-xs font-bold text-notion-text-secondary/50 dark:text-white/50">{count} {t('projects').toLowerCase()}</span>
    </div>
  </div>
);
