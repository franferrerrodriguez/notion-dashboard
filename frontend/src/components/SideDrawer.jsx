...
const SideDrawer = ({ itemId, type = 'project', onClose, projects = [], onProjectClick }) => {
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="grow space-y-1">
                        <p
                          className={`text-[13px] text-notion-text dark:text-white/90 leading-relaxed font-medium transition-all ${expandedItems.has(item.id) ? '' : 'line-clamp-3'}`}
                        >
  ...
  return (
    <div className="fixed inset-0 z-150 overflow-hidden font-sans">
      ...
      {/* Drawer Panel */}
      <div
        className={`absolute right-0 top-0 h-full w-full max-w-4xl bg-white dark:bg-notion-dark shadow-2xl transform transition-transform duration-500 ease-out border-l border-notion-border dark:border-white/10 ${itemId ? 'translate-x-0' : 'translate-x-full'}`}
      >
        {/* Header */}
        <div className="sticky top-0 z-10 bg-white/95 dark:bg-notion-dark/95 backdrop-blur-sm border-b border-notion-border dark:border-white/10 px-8 py-6 flex justify-between items-center text-notion-text dark:text-white">
          <div className="flex items-center gap-3">
            <h2 className="text-xl font-bold truncate max-w-md">
              {loading
                ? t('loading')
                : identification.name || (isTask ? t('linked_task') : 'Details')}
            </h2>
          </div>
          <button
            onClick={onClose}
                  ...
                  {isTask ? (
                    <>
                      <PropertyRow
                        ...
                        label={t('task_status')}
                        value={status.main?.name}
                        isTag
                        tagColor={status.main?.color}
                        isDark={isDark}
                      />
                      <PropertyRow
                        icon={<Target className="w-4 h-4" />}
                        label={t('task_priority')}
                        value={status.priority?.name}
                        isTag
                        tagColor={status.priority?.color}
                        isDark={isDark}
                      />
                      <PropertyRow
                        icon={<CalendarDays className="w-4 h-4" />}
                        label={t('task_due_date')}
                        value={item.date}
                      />
                      <PropertyRow
                        icon={<Castle className="w-4 h-4" />}
                    ...
                    <>
                      ...
                      <div className="flex items-start gap-4 py-1.5 group/prop">
                        ...
                        <div className="grow flex items-center gap-4">
                          <span className="text-xs font-medium text-notion-text dark:text-white/80 w-12 shrink-0">
                      ...
                      <PropertyRow
                        ...
                        label={t('prop_total_offered')}
                        value={formatCurrency(financials.totalOffered)}
                        isCurrency
                      />
                      <PropertyRow
                        icon={<Receipt className="w-4 h-4" />}
                        label={t('prop_total_billed')}
                        value={formatCurrency(financials.totalBilled)}
                        isCurrency
                      />
                      <PropertyRow
                        icon={<Receipt className="w-4 h-4" />}
                        label={t('prop_total_pend')}
                      ...
                      <div className="flex items-start gap-4 py-1.5 group/prop">
                        ...
                        <div className="grow flex items-center gap-4">
                          <span className="text-xs font-medium text-notion-text dark:text-white/80 w-12 shrink-0">
                            {(financials.billingPercentage || 0).toFixed(0)}%
                          </span>
                          <div className="grow max-w-[120px]">
                            <ProgressBar
                              value={financials.billingPercentage || 0}
                              color="#2ea043"
                            />
                          </div>
                        </div>
                      ...
                      <PropertyRow
                        icon={<Target className="w-4 h-4" />}
                        label={t('prop_offer_ref')}
                        value={assets.offerLink}
                        isLink
                      />
                    </>
                  )}
  ...
  return sortedGroups.map((group) => {
    ...
    return (
      <div key={group.name} className="ml-4">
        ...
        >
          <ChevronRight
            className={`w-3 h-3 text-notion-text-secondary dark:text-gray-500 transition-transform duration-200 ${isCollapsed ? 'rotate-0' : 'rotate-90'}`}
          />
          <span
            className={`text-[11px] font-bold px-2 py-0.5 rounded-full`}
            style={notionColors(group.color)}
          >
            {group.name}
          </span>
        </button>
            ...
            <table className="w-full text-left border-collapse min-w-[500px]">
              <thead>
                <tr className="border-b border-notion-border dark:border-white/5">
                  ...
                  <th className="py-2 text-[10px] font-bold text-notion-text-secondary dark:text-gray-500 uppercase tracking-wider flex items-center gap-3">
                    {' '}
                    {t('task_due_date')}
                  </th>
                  <th className="py-2 text-[10px] font-bold text-notion-text-secondary dark:text-gray-500 uppercase tracking-wider">
              ...
              <tbody>
                {group.tasks.map((task) => (
                  <tr
                    key={task.id}
                    className="border-b border-notion-border/30 dark:border-white/5 hover:bg-black/5 dark:hover:bg-white/2 transition-colors group/row"
                  >
                    <td className="py-3 pr-4 text-sm font-medium">
                      {task.properties?.['Nombre de la tarea']?.title?.[0]?.plain_text ||
                        t('linked_task')}
                    </td>
                    <td className="py-3 pr-4">
                      <span
                        className={`text-[10px] font-bold px-2 py-0.5 rounded-full`}
                        style={notionColors(task.properties?.Estado?.status?.color)}
                      >
                        {task.properties?.Estado?.status?.name}
                      </span>
                    </td>
                    <td className="py-3 pr-4 text-[11px]">
                      {task.properties?.['Fecha límite']?.date?.start || '-'}
                    </td>
                    <td className="py-3">
                      <span
                        className={`text-[10px] font-bold px-2 py-0.5 rounded-md`}
                        style={notionColors(task.properties?.Prioridad?.select?.color)}
                      >
        ...
        {contacts.map((contact) => (
          <tr key={contact.id} className="hover:bg-black/5 dark:hover:bg-white/2 transition-colors">
            ...
            <td className="py-3 px-4">
              {contact.role && (
                <span
                  className="px-1.5 py-0.5 rounded-sm text-[12px] font-medium"
                  style={{
                    background: getNotionColor(contact.role.color, true, isDark),
                    color: getNotionColor(contact.role.color, false, isDark),
                  }}
                >
                  {contact.role.name}
                </span>
            ...
            <td className="py-3 px-4 text-[11px] opacity-60 truncate max-w-[200px]">
              {contact.notes || '-'}
            </td>
          </tr>
        ))}
...
const PropertyRow = ({
  ...
  return (
    <div className="flex items-start gap-4 py-1.5 group/prop">
      <div className="flex items-center gap-2 w-36 shrink-0 text-notion-text-secondary dark:text-gray-400">
        <span className="text-sm opacity-70 group-hover/prop:opacity-100 transition-opacity">
          {icon}
        </span>
      ...
      <div className="grow flex items-center">
        {isTag ? (
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-[13px] text-notion-text dark:text-white/85 font-medium">
              {value}
            </span>
          </div>
        ) : isNotionFile ? (
          ...
          <span
            className={`text-[13px] font-medium ${isCurrency ? 'text-notion-text dark:text-white/95' : 'text-notion-text dark:text-white/85'} ${value === 'Empty' ? 'text-notion-text-secondary/40' : ''}`}
          >
            {value}
          </span>
        )}
